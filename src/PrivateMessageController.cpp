...
PrivateMessageController::PrivateMessageController(QObject *parent)
	: QObject(parent), m_ListView(NULL), m_Datas(new QList<PrivateMessageListItem*>()), m_RenderOption(IN_APP) {
...
void PrivateMessageController::checkErrorMessage(const QString &page) {
	...
	if(m_RenderOption == IN_APP) {
	    ...
	    qDebug() << "Autentification may have failed...";
	    emit autentificationFailed();
	}
